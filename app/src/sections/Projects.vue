<template>
    <section id="projects" ref="sectionEl" class="relative overflow-hidden py-[var(--section-space)]">
        <div class="section-shell relative z-10">
            <header class="projects-heading mb-16 grid gap-7 border-t border-white/10 pt-6 md:mb-24 md:items-end">
            ...
            <div v-else-if="displayedProjects.length" class="projects-list">
                <article v-for="(project, index) in displayedProjects" :key="project.id"
                    class="project-story group grid gap-7 border-t border-white/10 py-12 md:grid-cols-12 md:gap-10 md:py-20">
                    <router-link :to="'/project/' + project.id"
                        class="project-media relative block aspect-[4/3] overflow-hidden bg-surface md:col-span-7"
                        :class="index % 2 ? 'md:order-2' : ''">
                        <img v-if="project.imageUrl" :src="getImageUrl(project.imageUrl)" :alt="project.title"
                            class="project-image h-full w-full object-cover transition-transform duration-700 group-hover:scale-[1.035]" />
                        <div v-else
                            class="grid h-full place-items-center border border-white/5 text-2xl font-bold text-white/20">
                            {{ project.title }}</div>
                        <span
                            class="absolute right-4 top-4 grid h-12 w-12 place-items-center rounded-full bg-background/85 text-lg text-primary backdrop-blur transition-colors group-hover:bg-accent group-hover:text-background"
                            aria-hidden="true">↗</span>
                    </router-link>

                    <div class="project-copy flex flex-col justify-between md:col-span-5"
                        :class="index % 2 ? 'md:order-1 md:pr-8' : 'md:pl-8'">
                        <div>
                            <div
                                class="mb-7 flex items-center justify-between gap-4 font-mono text-xs uppercase tracking-[0.14em]">
                                <span class="text-accent">{{ projectNumber(index) }}</span><span
                                    class="text-secondary">{{ project.category }} · {{ project.year }}</span>
                            </div>
                            <h3
                                class="text-balance font-heading text-3xl font-bold leading-tight text-primary md:text-5xl">
                                <router-link :to="'/project/' + project.id"
                                    class="transition-colors hover:text-accent">{{ project.title }}</router-link>
                            </h3>
                            <div class="mt-7 border-l border-white/15 pl-5">
                                ...
                                <p class="line-clamp-3 leading-relaxed text-secondary">{{ project.contribution ||
                        ...
                        <div class="mt-8">
                            ...
                            <div class="flex flex-wrap gap-5 text-sm font-medium">
                                <router-link :to="'/project/' + project.id"
                                    class="text-primary underline decoration-white/20 underline-offset-8 transition-colors hover:text-accent">View
